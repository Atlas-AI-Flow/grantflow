...
def build_index(grants, grant_slugs):
    ...
    filter_buttons_sidebar = ""
    for niche in niches_found:
        ni = get_niche_info(niche)
        filter_buttons_sidebar += f'''
        <button onclick="filterNiche(\'{niche}\')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-sm font-medium text-gray-600 hover:text-teal-700 transition flex items-center justify-between group active-filter-btn" data-niche="{niche}">
            <span>{ni["emoji"]} {ni["label"]}</span>
            <span class="bg-gray-100 text-gray-500 text-xs py-0.5 px-2 rounded-full group-hover:bg-teal-50 group-hover:text-teal-700">{sum(1 for g in grants if g["niche"] == niche)}</span>
        </button>'''
    ...
    for niche, count in sorted(niche_counts.items()):
        ni = get_niche_info(niche)
        stats_html += f'<div class="bg-white border border-gray-100 rounded-xl p-3 text-center shadow-sm"><p class="text-2xl font-bold text-{ni["color"]}-600">{count}</p><p class="text-[10px] font-semibold uppercase text-gray-400 mt-1 tracking-wide">{ni["label"]}</p></div>\n'
...
<body class="bg-slate-50 text-slate-800 min-h-screen">
    ...
    <header class="bg-white border-b border-gray-200">
        <div class="max-w-4xl mx-auto px-4 py-12 text-center">
            ...
            <h1 class="text-3xl md:text-5xl font-bold text-slate-900 mb-4 tracking-tight">Find funding for your practice<br><span class="text-teal-600">or your family.</span></h1>
            <p class="text-lg text-slate-500 max-w-2xl mx-auto mb-6 leading-relaxed">A curated, searchable directory of grants, scholarships, and financial aid for PT, OT, SLP professionals and families.</p>
    ...
    <main class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <!-- Left Sidebar (Filters + Monetization) -->
            <aside class="w-full lg:w-64 flex-shrink-0 space-y-8">
                
                <!-- Filter Widget -->
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 sticky top-24 z-30">
                    <h3 class="font-bold text-slate-800 mb-4 text-sm uppercase tracking-wide text-gray-400">Filter by Category</h3>
                    <div class="space-y-1">
                        <button onclick="filterNiche('ALL')" class="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 text-sm font-medium text-gray-600 hover:text-teal-700 transition flex items-center justify-between group active-filter-btn" data-niche="ALL">
                            <span>All Grants</span>
                            <span class="bg-gray-100 text-gray-500 text-xs py-0.5 px-2 rounded-full group-hover:bg-teal-50 group-hover:text-teal-700">{total}</span>
                        </button>
                        {filter_buttons_sidebar}
                    </div>
                </div>

                <!-- Alert Widget (Monetization #1) -->
                <div class="bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl shadow-md p-5 text-white relative overflow-hidden">
                    <div class="absolute top-0 right-0 p-2 opacity-10 text-4xl transform rotate-12">🔔</div>
                    <div class="relative z-10">
                        <h3 class="font-bold mb-1">Never Miss a Grant</h3>
                        <p class="text-xs text-slate-300 mb-4 leading-relaxed">Get notified the moment a new grant drops for your specialty.</p>
                        <form onsubmit="event.preventDefault(); alert('Thanks for your interest! We are launching alerts next week. We will notify you.');" class="space-y-2">
                            <select class="w-full bg-slate-700/50 border border-slate-600 rounded-lg text-sm px-3 py-2 text-white focus:outline-none focus:border-teal-500 transition">
                                <option>Select Specialty...</option>
                                <option>Speech (SLP)</option>
                                <option>Occupational (OT)</option>
                                <option>Physical (PT)</option>
                                <option>Family / Special Needs</option>
                            </select>
                            <input type="email" placeholder="Enter your email" required class="w-full bg-slate-700/50 border border-slate-600 rounded-lg text-sm px-3 py-2 text-white placeholder-slate-400 focus:outline-none focus:border-teal-500 transition">
                            <button type="submit" class="w-full bg-teal-500 hover:bg-teal-400 text-white font-bold py-2 rounded-lg text-sm transition shadow-lg shadow-teal-500/20">Get Alerts &rarr;</button>
                        </form>
                        <p class="text-[10px] text-slate-500 mt-3 text-center">No spam. Unsubscribe anytime.</p>
                    </div>
                </div>

                <!-- Tracker Widget (Monetization #2) -->
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 relative overflow-hidden group hover:border-teal-200 transition">
                    <div class="absolute top-0 right-0 p-2 opacity-5 text-4xl transform -rotate-12 group-hover:scale-110 transition">📝</div>
                    <div class="relative z-10">
                        <h3 class="font-bold text-slate-800 mb-1">Grant Tracker</h3>
                        <p class="text-xs text-slate-500 mb-4 leading-relaxed">Keep track of deadlines, requirements, and logins in one place.</p>
                        <button onclick="alert('The Grant Tracker Notion Template is coming soon!')" class="w-full border border-slate-200 hover:border-teal-500 text-slate-600 hover:text-teal-700 font-semibold py-2 rounded-lg text-sm transition bg-gray-50 hover:bg-white flex items-center justify-center gap-2">
                            <span>Download Template</span>
                            <span class="text-xs bg-teal-100 text-teal-800 px-1.5 rounded">Free</span>
                        </button>
                    </div>
                </div>
            ...
            <div class="flex-1">
                ...
                <div class="relative mb-6">
                    ...
                    <input type="text" id="search" placeholder="Search grants by name, source, or keywords..." oninput="filterAll()" class="w-full pl-10 border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent shadow-sm transition">
                </div>

                <!-- Grid -->
                <div id="grid" class="grid gap-4 md:grid-cols-2 xl:grid-cols-2">
                    {cards_html}
                </div>
    ...
    <footer class="bg-white border-t border-gray-200 py-12 mt-12">
        <div class="max-w-7xl mx-auto px-4 text-center text-sm text-gray-500">
            ...
            <div class="mt-4 flex justify-center gap-4">
                <a href="#" class="text-gray-400 hover:text-teal-600">Privacy Policy</a>
                <a href="#" class="text-gray-400 hover:text-teal-600">Terms of Service</a>
                <a href="#" class="text-gray-400 hover:text-teal-600">Contact</a>
            </div>
        </div>
    </footer>
    ...
    <script>
        ...
        function filterNiche(niche) {{
            ...
            document.querySelectorAll('button').forEach(btn => {{
                ...
                if (btn.classList.contains('active-filter-btn')) {{
                     btn.classList.remove('bg-teal-50', 'text-teal-700', 'ring-1', 'ring-teal-200');
                     btn.classList.add('hover:bg-gray-50', 'text-gray-600');
                }}
                
                // Set active
                if ((niche === 'ALL' && btn.dataset.niche === 'ALL') || (btn.dataset.niche === niche)) {{
                    btn.classList.remove('hover:bg-gray-50', 'text-gray-600');
                    btn.classList.add('bg-teal-50', 'text-teal-700', 'ring-1', 'ring-teal-200');
                }}
            }});
